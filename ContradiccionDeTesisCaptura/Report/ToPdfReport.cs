...
namespace ContradiccionDeTesisCaptura.Report
{
    public class ToPdfReport
    {
        ...
        {
            myDocument = new Document(new Rectangle(288f, 144f), 15, 15, 30, 30);
            ...
            foreach (Contradicciones contra in listadoContradicciones.Listado)
            {
                ...
                cell = this.InitializeCell(String.Format("{0} {1}/{2}", asunto, contra.ExpedienteNumero, contra.ExpedienteAnio), Fuentes.ContenidoCelda, 1);
                table.AddCell(cell);
                ...
                {
                    Paragraph parat = new Paragraph(criterio.Criterio, Fuentes.ContenidoCelda) { Alignment = 3 };
        ...
        private PdfPCell InitializeCell(string cellContent, Font cellFont, int textAlign)
        {
            PdfPCell cell = new PdfPCell(new Phrase(cellContent, cellFont))
            {
                ...
                BorderWidthTop = 1,
                BorderWidthBottom = 1,
                HorizontalAlignment = textAlign /*0=Left, 1=Centre, 2=Right, 3=Justify*/
            };
        ...
        private PdfPCell InitializeCell(object enumer, Font cellFont, int textAlign)
        {
            ...
            if (enumer is ObservableCollection<Criterios>)
            {
                ...
                foreach (Criterios criterio in enumer as ObservableCollection<Criterios>)
                {
                    ...
                    Paragraph parart = new Paragraph(organoC, cellFont) { Alignment = textAlign };
            ...
            else if (enumer is ObservableCollection<PResolutivos>)
            {
                ...
                {
                    Paragraph parat = new Paragraph(res.Resolutivo, Fuentes.ContenidoCelda) { Alignment = textAlign };
                    cell.AddElement(parat);
                    cell.AddElement(new Paragraph(" ", Fuentes.ContenidoCelda));
                }
            }
            else if (enumer is ObservableCollection<Tesis>)
            {
                ...
                {
                    Paragraph parargr = new Paragraph(tes.ClaveIdentificacion, Fuentes.ContenidoCelda) { Alignment = textAlign };
                    cell.AddElement(parargr);
                    cell.AddElement(new Paragraph(" ", Fuentes.ContenidoCelda));

                    parargr = new Paragraph(tes.Rubro, Fuentes.ContenidoCelda) { Alignment = textAlign };
                    cell.AddElement(parargr);
                    cell.AddElement(new Paragraph(" ", Fuentes.ContenidoCelda));

                    parargr = new Paragraph("REG. IUS  ", Fuentes.ContenidoCelda) { Alignment = textAlign };
                    cell.AddElement(parargr);
                    cell.AddElement(new Paragraph(" ", Fuentes.ContenidoCelda));
                }
            }
            else if (enumer is ObservableCollection<Oficios>)
            {
                ...
                foreach (Oficios oficio in enumer as ObservableCollection<Oficios>)
                {
                    if (oficio.Oficio.Contains("CORREO") || oficio.FechaOficio == null)
                        ...
                        parat = new Paragraph(String.Format("{0}, DE {1}", oficio.Oficio, DateTimeUtilities.ToLongDateFormat(oficio.FechaOficio).ToUpper()), Fuentes.ContenidoCelda);
